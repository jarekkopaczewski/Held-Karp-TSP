...
pair<int[2], string[2]>* DataReader::readInitData()
{
    int size = 0;
    int test = 0, solution = 0;
    string name = "", path = "";
    pair<int[2], string[2]>* init = nullptr;
    ...
    if (fileData.good())
    {
        fileData >> size >> solution >> name >> path;
        init = new pair<int[2], string[2]>[size];
        init[0].first[0] = size;
        ...
        {
            fileData >> test >> solution >> name >> path;
            init[x].first[0] = test;
            init[x].first[1] = solution;
            init[x].second[0] = name;
            init[x].second[1] = path;
            cout << init[x].first[0] << ", " << init[x].first[1] << ", " << init[x].second[0] << ", " << init[x].second[1] << endl;
        }
    }
    else
    {
